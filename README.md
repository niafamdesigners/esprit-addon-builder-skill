...
این یک برنامه‌ی اجراشدنی نیست؛ «محصول» دو اسکیلِ مکمل است:

```text
skills/esprit-addon-builder/     # HTML → ساختار JSON افزونه
skills/esprit-addon-endpoints/   # ثبت آن ساختار از کنسول مرورگرِ پنل ادمین
```

اولی ساختار افزونه را از روی HTML می‌سازد؛ دومی یک مرجعِ بدون‌کدِ endpointهای ادمین
Esprit است و اسکریپتی برای کنسول مرورگر تولید می‌کند تا همان افزونه/جدول/فیلد/کوئری
را مستقیماً ثبت کند — بدون نیاز به مراجعه به سرور دیتابیس.
...
```text
.claude-plugin/
├── plugin.json                   # مانیفست پلاگین (نام، نسخه، نویسنده)
└── marketplace.json              # تعریف مارکت‌پلیس برای نصب از گیت‌هاب
skills/
├── esprit-addon-builder/         # اسکیل ۱: HTML → ساختار JSON افزونه
...
└── esprit-addon-endpoints/       # اسکیل ۲: تولید اسکریپت کنسول برای ثبت افزونه
    └── SKILL.md                  # endpointهای ادمین، الگوی CSRF و کدگذاری B64
...
### نصب به‌عنوان پلاگین (از گیت‌هاب) — روش پیشنهادی

این مخزن یک **marketplace** پلاگین Claude Code است (فایل‌های
`.claude-plugin/marketplace.json` و `.claude-plugin/plugin.json` در ریشه)، پس
می‌توانید آن را مستقیماً از روی لینک گیت‌هاب نصب کنید:

```text
/plugin marketplace add niafamdesigners/esprit-addon-builder-skill
/plugin install esprit-addon-builder@esprit-addon-builder-marketplace
```

یا از منوی `/plugin` → **Manage plugins** همان آدرس `owner/repo` را به‌عنوان
مارکت‌پلیس اضافه کنید و سپس **Install** را بزنید. پس از نصب، اسکیل به‌صورت خودکار
کشف می‌شود و با درخواست‌های مرتبط (تبدیل HTML به افزونه‌ی Esprit) فعال می‌گردد.

> نکته: برای نصب با لینک ساده‌ی `owner/repo`، مخزن باید public باشد. به‌روزرسانی
> نسخه‌ی جدید با `/plugin update` انجام می‌شود.

### در Claude Code (بدون پلاگین — کپی محلی)
...
Codex فایل **`AGENTS.md`** در ریشه‌ی مخزن را به‌صورت خودکار به‌عنوان دستورالعمل
پروژه می‌خواند؛ پس قواعد و گردش‌کار همین اسکیل به‌طور خودکار اعمال می‌شوند.

1. مخزن را به‌عنوان workspace به Codex بدهید (نیازی به کپی جداگانه نیست؛ همین که
   `AGENTS.md` در ریشه باشد کافی است).
2. فایل HTML طرح را در مخزن قرار دهید (مثلاً `index-to-convert.html`).
3. درخواست را مطرح کنید — همان `default_prompt` که در
   `skills/esprit-addon-builder/agents/openai.yaml` آمده:

   > Use the local Esprit Addon Builder skill to convert index-to-convert.html into reviewed addon JSON.

4. Codex فایل `SKILL.md` و referenceهای لازم را می‌خواند، در ابهام سؤال می‌پرسد و
   **JSON افزونه** را تولید می‌کند؛ SQL فقط در صورت درخواست export.

> فایل `agents/openai.yaml` متادیتای رابط (نام نمایشی، توضیح کوتاه و prompt
> پیش‌فرض) را برای ثبت اسکیل به‌صورت یک agent سبک در اختیار می‌گذارد.
